...
class PDownstreamEvaluatorBootstrap(DownstreamEvaluator):
    """Downstream Task: MoCo with Bootstrap Aggregation."""
    ...
    def __init__(self,
        ...
        super(PDownstreamEvaluatorBootstrap, self).__init__(
    ...
    def start_task(self, global_model):
        ...
        if self.task == "recon":
            logging.info("[DownstreamEvaluator::ERROR]: Please use the "
                         "downstream evaluator dedicated to the reconstruction"
                         " task.")
        elif self.task == "moco":
    ...
    def test_moco(self, global_model):
        """Validation of downstream task motion correction via bootstrap
        aggregation."""
                        ...
                        if idx % 2 == 0 and i % 2 == 0:  # plot some images
                            for agg in self.img_corr.keys():
                                log_images_to_wandb(
                                    ...
                                    wandb_log_name="{}/{}/MoCo_Examples_{}"
                                                   "/" .format(
                                        self.name, agg, dataset_key
                                    ),
                                    count=count,
                                    captions=["MoCo",
                                              "Motion-free",
                                ...
                                plot2wandb(
                                    maps=[self.t2star_uncorr_reg[i],
                                    self.t2star_hrqrcorr_reg[i],
                                    self.t2star_corr_reg[agg][i]],
                                    ref_map=self.t2star_gt[i, 0],
                                    titles=["Motion-corrupted",
                                            "Correction w\n HR \& QR",
                                            "PHIMO\n (Proposed)",
                                            "Motion-free"],
                                    wandb_log_name="{}/{}/Quantitative_MoCo_"
                                                   "Examples_{}/{}".format(
                                        self.name, agg, dataset_key,
                                        str(count)
                                    )
    ...
    def _process_motion_input_data(self, device, data):
        ...
        if isinstance(data[9][0], torch.Tensor):
            self.var_acc_rate = [detach_torch(item[0]) for item in data[9]]
        else:
            self.var_acc_rate = detach_torch(data[9][0])
    ...
    def _reconstruct_bootstrap_sample(self):
        ...
        prediction = forward_pass(
            self.model, self.hyper_setup, img_cc_zf,
            kspace_zf, mask, self.sens_maps,
            continuous_excl_rate=self.continuous_excl_rate
        )
